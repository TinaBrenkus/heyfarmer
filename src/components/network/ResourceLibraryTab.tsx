    ...
    return (
      <div className="flex items-center justify-center py-12">
        <div className="text-center">
          ...
          <p className="text-soil-500">Loading resources...</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h2 className="text-2xl font-bold text-soil-800 mb-2">Resource Library</h2>
          <p className="text-soil-500">Trusted tools and information for Texas farmers</p>
        </div>
        {isAdmin && (
          <button
          ...
          {Object.values(groupedResources).map((group: any) => (
            <div key={group.id}>
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="text-xl font-semibold text-soil-800">{group.label}</h3>
                <span className="text-sm text-soil-400">({group.resources.length})</span>
              </div>
              <div className="bg-white rounded-lg shadow-sm divide-y divide-gray-100">
                {group.resources.map((resource: Resource) => (
                  <button
                    ...
                    className="w-full flex items-center justify-between p-4 hover:bg-soil-50 transition-colors text-left group"
                  >
                    <div className="flex-1 min-w-0">
                      <h4 className="font-medium text-soil-800 group-hover:text-farm-green-800 transition-colors mb-1">
                        {resource.title}
                      </h4>
                      <p className="text-sm text-soil-500">{resource.description}</p>
                    ...
                    <ExternalLink className="h-5 w-5 text-soil-400 group-hover:text-farm-green-800 flex-shrink-0 ml-4" />
                  </button>
                ))}
              </div>
            </div>
