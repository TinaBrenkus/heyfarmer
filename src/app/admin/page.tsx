    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-soil-500">Loading admin dashboard...</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-soil-50">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            ...
            <button 
              ...
              className="px-4 py-2 text-soil-700 hover:text-soil-800 transition-colors"
            >
              Back to Dashboard
            </button>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {stats && (
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
            <div className="bg-white rounded-lg shadow-sm p-6">
              <div className="flex items-center">
                <Users className="h-8 w-8" style={{ color: '#4A5E35' }} />
                <div className="ml-4">
                  <p className="text-sm font-medium text-soil-400">Total Signups</p>
                  <p className="text-2xl font-bold text-soil-800">{stats.total}</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-lg shadow-sm p-6">
              <div className="flex items-center">
                <ShoppingCart className="h-8 w-8" style={{ color: '#C4622D' }} />
                <div className="ml-4">
                  <p className="text-sm font-medium text-soil-400">Consumers</p>
                  <p className="text-2xl font-bold text-soil-800">{stats.consumers}</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-lg shadow-sm p-6">
              <div className="flex items-center">
                <FarmLogo className="h-8 w-8" style={{ color: '#6B7F4A' }} />
                <div className="ml-4">
            ...
            <div className="bg-white rounded-lg shadow-sm p-6">
              <div className="flex items-center">
                <Tractor className="h-8 w-8" style={{ color: '#344628' }} />
                <div className="ml-4">
                  <p className="text-sm font-medium text-soil-400">Farmers</p>
                  <p className="text-2xl font-bold text-soil-800">{stats.market_gardeners + stats.production_farmers}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div className="flex flex-wrap items-center gap-4">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-medium text-soil-700">Filters:</span>
              </div>

              <select
                value={filters.userType}
                onChange={(e) => setFilters(prev => ({ ...prev, userType: e.target.value }))}
                className="px-3 py-1 border border-warm-border rounded-md text-sm"
              >
        ...
        {stats && (
          <div className="bg-white rounded-lg shadow-sm p-6 mb-8">
            <h3 className="text-lg font-semibold text-soil-800 mb-4">Signups by County</h3>
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
              {Object.entries(stats.by_county).map(([county, count]) => (
                <div key={county} className="text-center p-3 rounded-lg" style={{ backgroundColor: '#F0EDE4' }}>
                  <p className="text-sm font-medium text-soil-700">{county}</p>
                  <p className="text-lg font-bold" style={{ color: '#4A5E35' }}>{count}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="bg-white rounded-lg shadow-sm overflow-hidden">
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-soil-50">
                <tr>
                  <th className="px-6 py-3 text-left text-xs font-medium text-soil-400 uppercase tracking-wider">
                    User
                  </th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-soil-400 uppercase tracking-wider">
                    Type
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredEntries.map((entry) => {
                  ...
                  return (
                    <tr key={entry.id} className="hover:bg-soil-50">
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center">
                          <Mail className="h-4 w-4 text-soil-400 mr-2" />
                          <span className="text-sm text-soil-800">{entry.email}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center">
                          <UserTypeIcon className="h-4 w-4 text-soil-400 mr-2" />
                          <span className="text-sm text-soil-800">{getUserTypeLabel(entry.user_type)}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center">
                          <MapPin className="h-4 w-4 text-soil-400 mr-2" />
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center">
                          <Calendar className="h-4 w-4 text-soil-400 mr-2" />
                          <span className="text-sm text-soil-800">
                            {new Date(entry.created_at).toLocaleDateString()}
                          </span>
                        </div>
          ...
          {filteredEntries.length === 0 && (
            <div className="text-center py-12">
              ...
              <p className="text-soil-400">No waitlist entries found</p>
            </div>
          )}
        </div>
      </div>
